...
// Directly embed the service account JSON to avoid all build-time resolution issues.
// This is the most robust method for Next.js server environments.
...
if (!admin.apps.length) {
    try {
        ...
        console.log(`Firebase Admin SDK initialized successfully from embedded credentials.`);
    } catch (error: any) {
        console.error('Firebase Admin SDK initialization error from embedded credentials:', error.message);
    }
}
