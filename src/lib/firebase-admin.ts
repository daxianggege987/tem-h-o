...
if (!admin.apps.length) {
  try {
    // The most critical part of the fix:
    // The private_key from the source file has mixed/malformed newlines.
    // This .replace() call reliably formats the key into the exact format
    // the Firebase Admin SDK requires, preventing the startup crash.
    ...
    // This helps in debugging if the issue is something other than the key format.
    console.error('CRITICAL: Firebase Admin SDK initialization failed.', error);
  }
}

// These are exported so that API routes can use them to interact with Firebase services.
// If initialization failed above, these will be based on a non-functional admin instance,
// but the core crash on startup will have been prevented and logged.
